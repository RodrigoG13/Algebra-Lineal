...
def imprime_soluciones_infinitas_io(matrix, num_param):
    comienzo = len(matrix[0]) - num_param
    p_l = 0
    simbolos = []
    expresiones = []
...
def imprime_soluciones_infinitas(matrix, num_param, dict_var):
    comienzo = len(matrix[0]) - num_param
    p_l = 0
    simbolos = []
    for i in range(97, 123):
    ...
    for i in range(0, len(matrix)):
        for j in range(0, len(matrix[0])):
            if i != j and matrix[i][j] != 0 and j not in dict_pl:
                ...
                #dict_pl[dict_var[j]] = simbolos[encontrados]
                encontrados += 1
    ...
    print(f"dict_pl = {dict_pl}")
    """"
    transpuesta = transpone_matrix(matrix)
    col_cero = [0 for i in range(0, len(transpuesta[0]))]
    if col_cero in transpuesta:
        num_colcero = transpuesta.count(col_cero)
    """
    ...
    for i in range(0, len(matrix)):
        ...
        for j in range(0, len(matrix[0])-1):
            ...
            #print(f"{-matrix[indice][p - 1]}{param[p_l]}", end="")
            #p_l = p_l + 1
        #print(f" expresion final = {expresion}")
        #print(f"dict_var[i] = {dict_var[i]}")
        expresiones[dict_var[i]] = expresion
        #print(f"EXPRESIONES = {expresiones}")
        expresion = 0
        #p_l = 0
    #dict_var = {v:k for k, v in dict_var.items()}
    ...
    if tranpuesta.count(col_ceros) > 1:
        ...
        for i in range(encontrados, num_param):
            ...
            k += 1
        """
        pos_col = tranpuesta.index(col_ceros)
        for i in range(encontrados, num_param):
            expresiones[i] = simbolos[i]
        """

    print(f"EXPRESIONES = {expresiones}")
    return simbolos, expresiones

def imprime_soluciones_infinitas1(matrix, literales, num_param, param):
    comienzo = len(matrix[0]) - num_param
    p_l = 0
    bucles = 0
    for indice, ecuacion in enumerate(matrix):
...
def sistema_ecs(matrix_aumentada, matrix_coeficientes, numero_ecs, numero_incognitas):
    ...
    if rango_aumentada == rango_coeficientes and rango_aumentada == numero_incognitas:
        ...
        try:
            ...
            pass
        """
        for llave, valor in dict_var.items():
            if llave != valor:
                intercambio_cols = True
        if intercambio_cols:
            dict_var = {v:k for k, v in dict_var.items()}
            vect_aux = transpone_matrix(matrix_aumentada)
            matrix_aumentada = []
            for llave, valor in sorted(dict_var.items()):
                matrix_aumentada.append(vect_aux[valor])
            matrix_aumentada.append(cols_cero)
            matrix_aumentada = transpone_matrix(matrix_aumentada)
        """
        ...
        print(f"dict_var es {dict_var}")
        if bandera != 1:
            print("Definir expresiones")
            simb, exp = imprime_soluciones_infinitas(matrix_aumentada, numero_parametros, dict_var)
        else:
            print("expresiones b1")
            simb, exp = expresiones_b1(matrix_aumentada, numero_parametros)
        return numero_parametros, simb, exp, bandera_colcero, filas_ceros, indices_colsceros, dict_var
        ...
        matrix_aumentada = pivote_distinto_cero(matrix_aumentada, numero_ecs, 0, 0)
        numero_ecsact = numero_ecs - f_eliminadas
        diagonaliza_abajo(matrix_aumentada, numero_ecsact, numero_incognitas, dict_var)
        matrix_coeficientes = []
