...
def diagonaliza_arriba(matrix, ecuaciones, variables):
    ...
    while pos_pivote >= 0:
        ...
        #print(f" matrix antes de piv = 0 {matrix}, valor pivote = {valor_pivote}")
        if valor_pivote == 0:
            pivote_distinto_cero(matrix, ecuaciones, pos_pivote, pos_pivote)
            valor_pivote = matrix[pos_pivote][pos_pivote]
            #print(f" matrix despues de piv = 0 {matrix}, valor pivote = {valor_pivote}")
        if valor_pivote == 0:
            #break
            bandera = 1
        ...
        if valor_pivote != 0:
            matrix[pos_pivote] = filapiv_uno(matrix[pos_pivote][:], pos_pivote)
            valor_pivote = matrix[pos_pivote][pos_pivote]
            #print(f" matrix en piv != 0 {matrix}")
        if bandera == 1:
...
def expresiones_b1(aumentada, num_param):
    #print(f"aumentada es {aumentada}")
    simbolos = []
    simbolos_uso = []
    expresiones = []
    expresion = 0
...
def sistema_ecs(matrix_aumentada, matrix_coeficientes, numero_ecs, numero_incognitas):
    ...
    """
    print(f"matrix aumentada {matrix_aumentada}")
    print(f"range aumentada {rango_aumentada}")
    print(f"range coef {rango_coeficientes}")
    """
    if rango_aumentada == rango_coeficientes and rango_aumentada == numero_incognitas:
        ...
        #print(f"num param {numero_parametros}")
        ...
        """
        transpuesta = transpone_matrix(matrix_aumentada)
        print(f"condicional {transpuesta.count(cols_cero) > 0}")
        if transpuesta.count(cols_cero) > 1:
            while transpuesta.count(cols_cero) > 1:
                indices_colsceros.append(transpuesta.index(cols_cero))
                indices_colsceros.append(transpuesta.index(cols_cero))
            ...
            bandera_colcero = True
        if transpuesta.count(cols_cero) > 1:
            for i in range(0, len(transpuesta) -1):
                if i == cols_cero:
                    indices_colsceros.append(transpuesta.index(cols_cero))
            while transpuesta.count(cols_cero) > 1:
                transpuesta.remove(cols_cero)
            ...
            bandera_colcero = True
        print(matrix_aumentada)
        print(f"num inc {numero_incognitas}")
        print(f"num ec {numero_ecsact}")
        """
