'''
        LIBRERÍA DE SOLUCIÓN DE SISTEMAS DE ECUACIONES POR GAUSS JORDAN - LI
...
def define_solucion(matrix, numero_incognitas):
    ...
    ceros = [0 for i in range(0, len(transpuesta[0]))]
    if ceros in matrix:
        num_colceros = matrix.count(ceros)
...
def sistema_ecs_li(matrix_aumentada, matrix_coeficientes, numero_ecs, numero_incognitas, opcion):
    """
    ...
    if matrix_aumentada[0][0] == 0:
        matrix_aumentada = intercambia_columnas(matrix_aumentada, 0)
