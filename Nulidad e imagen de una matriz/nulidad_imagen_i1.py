...
from gj_indlineal_i1 import *
import os
from sub_super import *
from gj_nulidad_i1 import *
...
def independencia_lineal(vectores):
    ...
    """
    #print(vectores)
    vectores = transpone_matrix(vectores)
...
def genera_vectores(num_pl, pl,expresiones, dict_var):
    """
    ...
    """
    print(f"num param = {num_pl}")
    print(f"param = {pl}")
    print(f"expresiones {expresiones}")
    print(f"Dict var {dict_var}")
    vect = []
    intercambio_cols = False
...
"""
    for llave, valor in dict_var.items():
        if llave != valor:
            ...
            break
    if intercambio_cols:
        vect_aux = transpone_matrix(vect)
        vect = []
        for variable in dict_var.values():
            vect.append(vect_aux[variable])
        vect = transpone_matrix(vect)
...
while repetir == "si":
    os.system("cls")
    print("\t\t\t\t\t***ESPACIO NULO E IMAGEN DE UNA MATRIZ***\n")
    ...
    if type(soluciones) == tuple:
        numero_parametros, simb, exp, bandera_colcero, filas_ceros, indices_colsceros, dict_var = soluciones
        vectores = genera_vectores(numero_parametros, simb, exp, dict_var)
        """
        if bandera_colcero:
            vectores_colcero = []
            filas_ceros.pop()
    ...
    print(f"Dimension imagen = {dimension_imagen}")
