...
def crear_guardar_grafico(df, titulo, min_x, max_x, min_y, max_y, un_solo_trazo):
    ...
    for i, vertice in enumerate(df.values):
        lb = next(etiquetas)
        #ax.annotate(text=lb+f"({vertice[0]:.2f},{vertice[1]:.2f})", xy=vertice + 0.1)
        #ax.annotate(text=f"({vertice[0]:.2f},{vertice[1]:.2f})", xy=vertice + 0.1)
    """
